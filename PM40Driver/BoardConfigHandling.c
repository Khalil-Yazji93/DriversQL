...
NTSTATUS GetBoardConfigDeviceControl(IN WDFDEVICE device, IN WDFREQUEST Request, IN size_t * pInfoSize)
{
       ...
       KdPrintEx((1, DPFLTR_ERROR_LEVEL, "--> GetBoardConfigDeviceControl, Request %p", Request));

        // Validate all the parameters.
        // Validate pInfoSize.
        if (pInfoSize == NULL) {
           KdPrintEx((1, DPFLTR_ERROR_LEVEL, "<-- DMADriverGetDeviceContext returned NULL"));
//               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "<-- DMADriverGetDeviceContext returned NULL"));
        ...
        if ((status = WdfRequestRetrieveOutputBuffer(Request, sizeof(BOARD_CONFIG_STRUCT),      /* Min size */
                                                     (PVOID *) & pBoardConfig,  /* buffer    */
                                                     NULL)) != STATUS_SUCCESS) {
               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "<-- WdfRequestRetrieveOutputBuffer failed 0x%x", status));
                return STATUS_INVALID_PARAMETER;
        }

        if ((status = DMADriverBoardConfigInit(pDevExt)) != STATUS_SUCCESS) {
               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "<-- DMADriverBoardConfigInit failed 0x%x", status));
                return status;
        }
       ...
       KdPrintEx((1, DPFLTR_ERROR_LEVEL,"<-- GetBoardConfigDeviceControl"));
...
NTSTATUS GetDmaEngineCapabilities(IN WDFDEVICE device, IN WDFREQUEST Request, IN size_t * pInfoSize)
{
        ...
        *pInfoSize = 0;
        ...
        status = WdfRequestRetrieveInputBuffer(Request, sizeof(UINT32), // Minimum size 
                                               (PVOID *) & pDmaEngine,  // Buffer
                                               NULL);
        if (status != STATUS_SUCCESS) {
               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "<-- WdfRequestRetrieveInputBuffer failed 0x%x", status));
                return status;
        }
        // Get the output buffer, where we store the DMA Capabilities info
        if ((status = WdfRequestRetrieveOutputBuffer(Request, sizeof(DMA_CAP_STRUCT),   // Minimum size 
...
NTSTATUS GetDmaPerfNumbers(IN WDFDEVICE device, IN WDFREQUEST Request, IN size_t * pInfoSize)
{
        ...
        *pInfoSize = 0;
        ...
        if (status != STATUS_SUCCESS) {
               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "DMA Engine number is invalid 0x%x", status));
                return status;
        }
        // Get the output buffer, where we store the DMA performance
        status = WdfRequestRetrieveOutputBuffer(Request, (sizeof(UINT64) * 3),  // Minimum size
        ...
        if (pDmaStatStruct != NULL) {
               ...
               KdPrintEx((1, DPFLTR_ERROR_LEVEL, "Invalid parameter"));
