...
NTSTATUS DMADriverWatchdogTimerInit(IN WDFDEVICE Device)
{
        ...
        WDF_TIMER_CONFIG_INIT_PERIODIC(&wdfTimerConfig, DMADriverWatchdogTimerCall, 990);       // one second
        // setup the driver object as the timer's parent object
        WDF_OBJECT_ATTRIBUTES_INIT(&timerAttributes);
        timerAttributes.ParentObject = pDevExt->Device;

        status = WdfTimerCreate(&wdfTimerConfig, &timerAttributes, &pDevExt->WatchdogTimer);
        return status;
}
 ...
 * \return none
 */
VOID DMADriverWatchdogTimerCall(IN WDFTIMER Timer)
{
        INT32 dmaEngine;
        PDEVICE_EXTENSION pDevExt;
        ...
        for (dmaEngine = 0; dmaEngine < MAX_NUM_DMA_ENGINES; dmaEngine++) {
                if (pDevExt->pDmaEngineDevExt[dmaEngine] != NULL) {
                        ...
                        pDevExt->pDmaEngineDevExt[dmaEngine]->HardwareTimeInLastSecond = (UINT64) pDmaExt->pDmaEng->DMAActiveTime;
                        pDevExt->pDmaEngineDevExt[dmaEngine]->DMAInactiveTime = (UINT64) pDmaExt->pDmaEng->DMAWaitTime;
                        pDevExt->pDmaEngineDevExt[dmaEngine]->BytesInLastSecond = (UINT64) pDmaExt->pDmaEng->DMACompletedByteCount;
...
VOID DMADriverWatchdogTimerStart(IN WDFDEVICE Device)
{
        ...
        if (pDevExt->WatchdogTimer) {
            WdfTimerStart(pDevExt->WatchdogTimer, WDF_REL_TIMEOUT_IN_MS(990));
        }
}
